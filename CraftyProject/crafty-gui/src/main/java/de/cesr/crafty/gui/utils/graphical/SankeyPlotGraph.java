	...
	public static SankeyPlot AFtsToSankeyPlot(HashMap<String, HashMap<String, Integer>> h,
		...
		h.forEach((sender, hash) -> {
			hash.forEach((reciever, value) -> {
				if (value > 0) {
					Color color = colors.get(reciever) != null ? colors.get(reciever) : Color.BLACK;
					PlotItem plotItem2 = new PlotItem(reciever, color, 1);
					plothash.put(reciever, plotItem2);
				}
			});
			hashItems.put(sender, plothash);
			Color color = colors.get(sender) != null ? colors.get(sender) : Color.BLACK;
			PlotItem plotItem1 = new PlotItem(sender, color, 0);
